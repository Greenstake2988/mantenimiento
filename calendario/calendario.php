...
function dame_estilo($dia_imprimir){
	...
	if ($dia_solo_hoy == $dia_imprimir && $mes==date("n", $tiempo_actual) && $ano==date("Y", $tiempo_actual)){
		...
		$fecha=date('Y-M-d',mktime(12,0,0,$mes,$dia_imprimir,$ano));
		if (date("w",$fecha)==0){
			//si es domingo 
...
function escribe_formulario_fecha_vacio($nombrecampo,$nombreformulario){
	global $raiz;
	$fecha=$_POST['fecha'];
 echo '<INPUT name="'.$nombrecampo.'" " size="10" readonly="" value='.date('Y/m/d').' >';
 echo'<BUTTON name="guarda"  type="button" onclick="muestraCalendario(\''. $raiz.'\',\''. $nombreformulario .'\',\''.$nombrecampo.'\')"> <IMG src="../imagenes/picker.gif"></BUTTON>';
}
?>
