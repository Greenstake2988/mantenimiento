  ...
  <tr>
    <td valign="top" bgcolor="#FFFFFF"><!-- InstanceBeginEditable name="EditRegion3" -->
        <form  name="form1"  method="post" action="../FormatoHTML/FormTipo1.php" >
          ...
          <div>
          <?php 
          if(($_SESSION['area']=='1' and $_SESSION['sol']=='cc') or ($_SESSION['area']=='10' and $_SESSION['sol']=='rms')) 
             { 
          ?>
	  ...
	  <?php
	      ...
	      if($_SESSION['sol']=='cc')
                  {
		  $consulta= mysql_query("SELECT max(folio+1) FROM solicitudes",$enchufarsolicitudes) or die ('Error en la conexioacute;con  el gestor al leer');
                  $folio = mysql_fetch_row($consulta);
                  echo' <input name="folio"  size="2" readonly="true"  value="'.$folio[0].'"/>';
                  $anio=date('/y');
                  }
               else
	           {
		//codigo ingresado el 19 de mayo de 2009 (codigo para recursos materiales)
                $consulta1= mysql_query("SELECT max(folio+1)  FROM solicitudes where solicita_a<>'cc'",$enchufarsolicitudes) or die ('Error en la conexioacute;con  el gestor al leer');
                $folio = mysql_fetch_row($consulta1);
                echo' <input name="folio"  size="2" readonly="true"  value="'.$folio[0].'"/>';
                $anio=date('/y');
                //fin codigo recursos materiales
                    }
?>
                <input name="anio" type="text"  value="<?php echo $anio;?>" size="2" readonly="true" /></td>
            </tr>
            <tr>
              <td><div align="center">FECHA</div></td>
              <td><input name="fecha_solicitud" type="text" value="<?php echo date('Y/m/d');?>" size="10" readonly="true" /></td>
            </tr>
            <tr>
		...
		$consulta= mysql_query("SELECT id_area,descripcion FROM area",$enchufarusuarios) or die ('Error en la conexioacute;con  el gestor al leer');
              ?>
                <select name="area">
              <?php
            ...
            <tr>
              ...
              <?php 
		include('../../conexion/enchufarusuarios.php');
		$consulta= mysql_query("SELECT id_usuario,nombres,apellidop,apellidom FROM usuarios",$enchufarusuarios) or die ('Error en la conexioacute;con  el gestor al leer');
              ?>
                <select name= "nombre">
             <?php
  ...
  <tr>
    <td bgcolor="#FFFFFF" class="textpie">
    <div align="center">
